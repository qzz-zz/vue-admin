<template>
	<div>
		<el-card class="box-card">
			<div slot="header">
				<span>商品分类</span>
			</div>
			<!-- 树形组件 -->
			<el-tree ref="tree" lazy :load="loadNode" node-key="id" :default-expanded-keys="[1]" :props="defaultProps">
				<span class="custom-tree-node" slot-scope="{ node, data }">
					<span>{{ node.label }}</span>
					<span>
						<el-button type="text" icon="el-icon-edit" :disabled="node.level==1" size="mini" @click.stop="() => openEditModal(node,data)">
							编辑
						</el-button>
						<el-button type="text" icon="el-icon-plus" size="mini" @click.stop="() => openInsertModal(node,data)">
							添加
						</el-button>
						<el-button type="text" icon="el-icon-delete" :disabled="node.level==1" size="mini" @click.stop="() => remove(node, data)">
							删除
						</el-button>
					</span>
				</span>
			</el-tree>
		</el-card>
		<!-- 添加弹窗 -->
		<el-dialog title="添加分类" :visible.sync="insertModalShow">
			<el-form :model="insertForm" :rules="rules" ref="insertForm" label-position="left">
				<el-form-item label="名称" prop="name" label-width="80px">
					<el-input v-model="insertForm.name" autocomplete="off"></el-input>
				</el-form-item>
				<el-form-item label="图片" prop="img" label-width="80px">
					<single-upload type="/api/upload/common" :url="insertForm.img" @success="handleUploadSuccess" @remove="insertForm.img='' "></single-upload>
				</el-form-item>
				<el-form-item class="footer">
					<el-button @click="insertModalShow = false">取 消</el-button>
					<el-button @click="handleInsertNode('insertForm')" type="primary">保 存</el-button>
				</el-form-item>
			</el-form>
		<!-- 	<div slot="footer" class="dialog-footer">
				<el-button @click="insertModalShow = false">取 消</el-button>
				<el-button type="primary" @click="handleInsertNode">添 加</el-button>
			</div> -->
		</el-dialog>
		<!-- 编辑弹窗 -->
		<el-dialog title="编辑分类" :visible.sync="editModalShow">
			<el-form :model="editForm" :rules="rules" ref="editForm" label-position="left">
				<el-form-item label="名称" prop="name" label-width="80px">
					<el-input v-model="editForm.name" autocomplete="off"></el-input>
				</el-form-item>
				<el-form-item label="图片" prop="img" label-width="80px">
					<single-upload type="/api/upload/common" :url="editForm.img" @success="handleUploadSuccess" @remove="editForm.img='' "></single-upload>
				</el-form-item>
				<!-- <div slot="footer" class="dialog-footer"> -->
				<el-form-item class="footer">
					<el-button @click="editModalShow = false">取 消</el-button>
					<el-button @click="handleUpdateNode('editForm')" type="primary">保 存</el-button>
				</el-form-item>
				<!-- </div> -->
			</el-form>
		</el-dialog>
	</div>

</template>

<script>
	import { Category } from '@/api/index';
	import SingleUpload from '@/components/SingleUpload.vue';
	export default {
		components: {
			SingleUpload
		},
		data() {
			return {
				defaultProps: {
					label: 'name'
				},
				insertForm: {
					name: '',
					pId: '',
					img: '',
				},
				editForm: {
					id: '',
					name: '',
					// pId: '',
					img: '',
				},
				rules: {
					name: [{
							required: true,
							message: '请输入分类名称',
							trigger: 'blur'
						},
						{
							min: 1,
							message: '长度不能少于1个字符',
							trigger: 'blur'
						}
					]
				},
				nodeData: {}, //节点数据
				node: {}, //节点
				insertModalShow: false,
				editModalShow: false,
			}
		},
		methods: {
			//获取分类
			async loadNode(node, resolve) {
				if (node.level == 0) {
					let { status, data } = await Category.subcate({ pId: 0 });
					if (status) {
						return resolve(data);
					}
				}
				//父级节点id
				let { id } = node.data;
				//获取子级节点
				let { status, data } = await Category.subcate({ pId: id });
				if (status) {
					return resolve(data);
				}
			},
			//打开编辑窗口
			openEditModal(node, data) {
				this.editForm = { ...data };
				this.node = node; //数据转存
				this.editModalShow = true;
			},
			//编辑节点
			handleUpdateNode(formName) {
				//1.表单验证
				this.$refs[formName].validate(async (valid) => {
					if (!valid) {
						//校验失败
						return false;
					}
					//2.ajax
					let { status, msg, data } = await Category.edit({ ...this.editForm });
					if (status) {
						this.$message.success("编辑成功！");
						//更新新的tree节点
						this.node.data = this.editForm;
						this.editModalShow = false;
					}else{
						this.$message.erro("编辑失败！");
					}
				});
			},

			//打开添加窗口
			openInsertModal(node, data) {
				this.insertForm = {name: '',pId: '',img: ''};
				this.node = node; //对node进行转存，方便下面插入节点使用
				this.insertForm.pId = data.id;
				// console.log(this.insertForm.parent_id);
				this.insertModalShow = true;
				// console.log(node,data)
				// console.log(this.insertForm.pId);
			},
			//插入节点
			handleInsertNode(formName) {
				//1.表单验证
				this.$refs[formName].validate(async (valid) => {
					if (!valid) {
						//校验失败
						return false;
					}
					//2.ajax
					let { status, msg, data } = await Category.insert({ ...this.insertForm });
					if (status) {
						this.$message.success("添加成功！");
						//插入新的tree节点
						this.$refs.tree.append({ ...data, ...this.insertForm }, this.node);
						this.insertModalShow = false;
					}else{
						this.$message.erro("添加失败！");
					}
				});
			},

			//删除节点
			remove(node, data) {
				let id = data.id;
				this.$confirm('此操作将永久删除该分类, 是否继续?', '提示', {
						type: 'warning'
					})
					.then(async () => {
						let { status } = await Category.remove({id});
						if (status) {
						this.$refs.tree.remove({ ...data });
						this.$message.success('删除成功！');
						}
					})
					.catch(() => {
						this.$message.info('已取消删除！');
					});
			},


			//图片上传成功
			handleUploadSuccess({ status, msg, src }) {
				this.insertForm.img = src;
				this.editForm.img = src;
			},
		}
	}
</script>

<style>
	.custom-tree-node {
		flex: 1;
		display: flex;
		align-items: center;
		justify-content: space-between;
		font-size: 14px;
		padding-right: 8px;
	}

	.footer {
		padding: 10px 20px 0 20px;
		text-align: right;
		box-sizing: border-box;
	}
</style>
